...
public class WalletController {
    ...
    @GetMapping(value = "/get_total_value_of_coins/{walletId}")
    public ResponseEntity<BaseResponse> getTotalValueOfCoinsFromWallet(@PathVariable("walletId") Long walletId){
        try {
            return new ResponseEntity<>(walletService.getTotalValueOfCoinsFromWallet(walletId),HttpStatus.OK);
        }catch (NotFoundException e){
            return new ResponseEntity<>(new ErrorResponse(e.getMessage()), HttpStatus.BAD_REQUEST);
        }
